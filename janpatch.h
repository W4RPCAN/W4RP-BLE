...
#if !defined(JANPATCH_STREAM) &&                                               \
    (defined(__unix__) || (defined(__APPLE__) && defined(__MACH__)))
#include <stdio.h>
#define JANPATCH_STREAM FILE
#elif !defined(JANPATCH_STREAM)
#error                                                                         \
    "JANPATCH_STREAM not defined, and not on POSIX system. Please specify the JANPATCH_STREAM macro"
#endif
...
typedef struct {
  ...
  // the combination of the size of both the source + patch files (that's the
  // max. the target file can be)
  long max_file_size;
} janpatch_ctx;
 ...
 */
static size_t jp_fread(janpatch_ctx *ctx, void *ptr, size_t size, size_t count,
                       janpatch_buffer *buffer) {
 ...
 */
static size_t jp_fwrite(janpatch_ctx *ctx, const void *ptr, size_t size,
                        size_t count, janpatch_buffer *buffer) {
  ctx->fseek(buffer->stream, buffer->position, SEEK_SET);
...
static int jp_fseek(janpatch_buffer *buffer, long int offset, int origin) {
  if (origin == SEEK_SET) {
    ...
    JANPATCH_ERROR(
        "Origin %d not supported in jp_fseek (only SEEK_CUR,SEEK_SET)\n",
        origin);
    return -1;
  }
  return 0;
...
static void jp_final_flush(janpatch_ctx *ctx, janpatch_buffer *buffer) {
  ...
  // this can happen when the last operation (e.g. jp_putc) has just crossed
  // page boundary
  if (page != buffer->current_page) {
    // flush the page buffer...
    if (buffer->current_page != 0xFFFFFFFF) {
...
static void process_mod(janpatch_ctx *ctx, janpatch_buffer *source,
                        janpatch_buffer *patch, janpatch_buffer *target,
                        bool up_source_stream) {
  // it can be that ESC character is actually in the data, but then it's
  // prefixed with another ESC so... we're looking for a lone ESC character
  size_t cnt = 0;
  while (1) {
    cnt++;
    int m = jp_getc(ctx, patch);
    if (m == -1) {
      // End of file stream... rewind 1 character and return, this will yield
      // back to janpatch main function, which will exit
    ...
    if (m == JANPATCH_OPERATION_ESC) {
      ...
      if (up_source_stream) {
        jp_fseek(source, 1, SEEK_CUR);
               ...
               m <= 0xA6) { // character after this is an operator? Then roll
                            // back two characters and exit
...
static int find_length(janpatch_ctx *ctx, janpatch_buffer *buffer) {
   ...
   * If byte[0] is 254 => use (byte[1] << 16) + (byte[2] << 8) + byte[3] (NOT
   * VERIFIED)
   */
  ...
  if (l <= 251) {
    ...
    return (jp_getc(ctx, buffer) << 24) + (jp_getc(ctx, buffer) << 16) +
           (jp_getc(ctx, buffer) << 8) + (jp_getc(ctx, buffer));
  } else {
  ...
  // it's fine if we get over the end of the stream here, will be caught by the
  // next function
}

int janpatch(janpatch_ctx ctx, JANPATCH_STREAM *source, JANPATCH_STREAM *patch,
  ...
  while ((c = jp_getc(&ctx, &ctx.patch_buffer)) != EOF) {
    ...
    case JANPATCH_OPERATION_EQL: {
      ...
      if (length == -1) {
        ...
        JANPATCH_ERROR("Positions are, source=%ld patch=%ld new=%ld\n",
                       ctx.source_buffer.position, ctx.patch_buffer.position,
                       ctx.target_buffer.position);
        return 1;
      }
    ...
    case JANPATCH_OPERATION_MOD: {
      ...
      // so just write everything (until the next ESC sequence) to the target
      // JANPATCH_STREAM but also up the position in the source JANPATCH_STREAM
      // every time
      process_mod(&ctx, &ctx.source_buffer, &ctx.patch_buffer,
                  &ctx.target_buffer, true);
      break;
    }
    case JANPATCH_OPERATION_INS: {
      JANPATCH_DEBUG("INS: ");
      // INS inserts the sequence in the new JANPATCH_STREAM, but does not up
      // the position of the source JANPATCH_STREAM so just write everything
      // (until the next ESC sequence) to the target JANPATCH_STREAM

      process_mod(&ctx, &ctx.source_buffer, &ctx.patch_buffer,
                  &ctx.target_buffer, false);
      break;
    }
    ...
    case -1: {
      // End of file stream... rewind 1 character and break, this will yield
      // back to main loop
    ...
    default: {
      ...
      // See:
      // https://github.com/janjongboom/janpatch/issues/16#issuecomment-705010958
